...
BigQueryLogViewer.Row = React.createClass
  ...
  componentDidMount: () ->
    React.findDOMNode(@).scrollInfoView if @props.highlighted
  ...
  render: ->
    ...
    if @props.type == 'results'
      return (
        <tr>
          <td className='column-controls' onClick={@handleShowProximity}><i className='fa fa-external-link'></i></td>
      ...
      return (
        <tr ref="row" className={'highlight-row' if @props.highlighted}>
          <td className='column-ts'>{@state.tss}<span className='ts-milliseconds'>.{@state.tsm}</span></td>
          <SeverityCell severity={row.severity}/>
          <td className='column-msg'><pre>{collapsedMsg}</pre></td>
        </tr>
